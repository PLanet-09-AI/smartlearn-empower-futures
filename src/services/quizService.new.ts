...
`;
      ...
      const quiz: Quiz = {
        ...
        questions: questionsData.map(q => ({
          ...
          explanations: q.options.reduce((acc, option, index) => {
            acc[index] = option.explanation;
            return acc;
    ...
    try {
      ...
      for (const answer of submission.questions) {
        ...
        if (question) {
          ...
          explanations[answer.questionId] = question.explanations?.[answer.selectedOptionId] || '';
    ...
    try {
      ...
      for (const [courseId, results] of courseGroups) {
        ...
        analytics.push({
          ...
          lastAttempt: new Date(Math.max(...results.map((r: any) => new Date(r.attemptedAt).getTime())))
        });
      }
